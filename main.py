...
class DB:
    ...
    def query(self, sql: str):
        with contextlib.closing(sqlite3.connect(self.db_name)) as con, con,  \
        ...
        with contextlib.closing(sqlite3.connect(self.db_name)) as con, con,  \
            ...
            for row in rows:
                ...
                if result > 0:
                    ...
                    cur.execute(f"INSERT INTO {tbname} VALUES({','.join(['?' for i in range(len(cols))])})", row)
                    inserted.append(row)
        return inserted
...
class LineWorker:
    ...
    def send_flex_message(self, to: str, alt_text: str, flex_message):
        id_ = self.get_id(to=to)
        self.api_client.push_message(id_, FlexSendMessage(alt_text=alt_text, contents=flex_message))
...
class ZSJHSCrawler(Crawler):
    ...
    def format_message(self, columns, row):
        ...
        template["body"]["contents"][1]["contents"][1]["text"] = row[columns.index("標題")]
        template["body"]["contents"][2]["contents"][1]["action"]["uri"] = self.get_shorten_url(row[columns.index("標題連結")])
        template["body"]["contents"][3]["contents"][1]["text"] = row[columns.index("發布日期")]
        return template
    
    def extract_columns(self, soup: BeautifulSoup, q: str, to_exclude: list):
        table = soup.find("table", {"summary": re.compile(q)})
        columns = [re.sub(r"\s+", "", th.text) for th in table.find('tr').find_all('th')]
    ...
    def extract_post_content(self, post_content_url: str):
        ...
        rows = [re.sub(r"\s+", "", soup.find("th", string = re.compile(c)).find_next_sibling("td").text) for c in columns]
        return columns, rows
    ...
    def main(self):
        ...
        if not inserted:
            logger.info("沒有找到中山國中相關的通知。")
        for row in inserted:
            self.line_worker.send_flex_message(
                to="group_chat",
                alt_text="羽球場地通知-中山國中",
                flex_message=self.format_message(cols, row)
            )
            logger.info(f'發送訊息: 標題: {row[cols.index("標題")]}, 發布日期: {row[cols.index("發布日期")]}')


if __name__ == "__main__":
    def run_all():
        try:
            logger.info(f"Run Scheduled Job: {datetime.datetime.now(pytz.timezone(CONFIG.get('tz')))}")
            zsjhs_crawler.main()
        except Exception as e:
            logger.exception(e)
    ...
    zsjhs_crawler = ZSJHSCrawler(db, line_worker)

    schedule.every(CONFIG.get("default_schedule_job_interval")).seconds.do(run_all)

    logger.info(f"Job Started At: {datetime.datetime.now(pytz.timezone(CONFIG.get('tz')))}")
    while True:
